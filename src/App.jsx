...
const DjSection = () => {
  ...
  return (
    ...
    >
      {/* Dates Column */}
      <div className="bg-bg-card/60 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 flex flex-col shadow-2xl hover:border-white/20 transition-all duration-500 h-[450px]">
        <div className="flex flex-col h-full">
          <div className="flex-shrink-0">
            <h3 className="text-accent-highlight text-xs font-black uppercase tracking-widest mb-3 flex items-center gap-2 border-b border-white/5 pb-2">
              <SafeIcon icon={FiCalendar} className="text-sm" />Prochaines Dates</h3>
            <div className="space-y-2">
              {futureDates.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 hover:border-accent-primary/30 transition-all group">
                  <div className="flex flex-col items-center justify-center bg-accent-primary/20 border border-accent-primary/30 rounded-md w-12 h-12 shrink-0 group-hover:scale-105 transition-transform">
      ...
      color: 'rgb(255, 255, 255)'
    }} className="text-white text-xs font-bold tracking-wide group-hover:text-accent-highlight transition-colors">{item.event}</span>
                    <span className="text-text-secondary text-[10px] font-medium bg-black/20 w-fit px-2 py-0.5 rounded mt-1">{item.genre}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="flex-1 flex flex-col min-h-0 mt-3">
            <h3 className="text-text-muted text-xs font-black uppercase tracking-widest mb-2 border-b border-white/5 pb-2 flex-shrink-0">Archives</h3>
            <div className="flex-1 overflow-y-auto pr-1 space-y-1 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent hover:scrollbar-thumb-white/20">
              {pastDates.map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-md hover:bg-white/5 transition-colors opacity-70 hover:opacity-100 group">
                  <span className="text-text-secondary text-xs font-mono min-w-[60px] text-right border-r border-white/10 pr-2">{item.date}</span>
                  <div className="flex flex-col">
                    <span className="text-text-primary text-xs font-bold group-hover:text-white">{item.event}</span>
                    <span className="text-text-muted text-[9px]">{item.genre}</span>
                  </div>
                </div>
              ))}
            </div>
